import type { LibrarySymbolInfo, SubscribeBarsCallback } from '../../../charting_library/datafeed-api';
import { getErrorMessage, logMessage } from './helpers';
import type { GetBarsResult, HistoryProvider } from './history-provider';
...
export class DataPulseProvider {
  ...
  private _updateDataForSubscriber(listenerGuid: string): Promise<void> {
    ...
    const rangeEndTime = Number.parseInt((Date.now() / 1000).toString(), 10);
...
function periodLengthSeconds(resolution: string, requiredPeriodsCount: number): number {
  ...
  if (resolution === 'D' || resolution === '1D') {
    ...
    daysCount = (requiredPeriodsCount * Number.parseInt(resolution, 10)) / (24 * 60);
  }
