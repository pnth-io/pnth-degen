...
import { getErrorMessage, logMessage, type RequestParams, type UdfErrorResponse } from './helpers';
import { type GetBarsResult, HistoryProvider, type PeriodParamsWithOptionalCountback } from './history-provider';
  ...
  public getServerTime(callback: ServerTimeCallback): void {
    ...
    this._send<string>('time')
      .then((response: string) => {
        const time = Number.parseInt(response, 10);
