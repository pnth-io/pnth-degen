import type { Bar, HistoryMetadata, LibrarySymbolInfo, PeriodParams } from '../../../charting_library/datafeed-api';
          ...
          if (response.s === 'no_data') {
            ...
            for (let i = 0; i < response.t.length; ++i) {
              const barValue: Bar = {
                ...
                close: Number.parseFloat(response.c[i]),
                open: Number.parseFloat(response.c[i]),
                high: Number.parseFloat(response.c[i]),
                low: Number.parseFloat(response.c[i]),
              };

              if (ohlPresent) {
                barValue.open = Number.parseFloat((response as HistoryFullDataResponse).o[i]);
                barValue.high = Number.parseFloat((response as HistoryFullDataResponse).h[i]);
                barValue.low = Number.parseFloat((response as HistoryFullDataResponse).l[i]);
              }

              if (volumePresent) {
                barValue.volume = Number.parseFloat((response as HistoryFullDataResponse).v[i]);
              }
